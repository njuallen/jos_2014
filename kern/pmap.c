...
boot_alloc(uint32_t n)
{
	...
	// end of the remapped physical memory
	// we only remapped 4MB of physical memory in entrypgdir.c
	char *remapped_phy_mem_top = (char *)0xf0400000;
	...
	// what would nextfree be if we allocate a new block
	// well, the addition may cause an overflow
	// but I do not want to deal with it

	char *new_nextfree = ROUNDUP((nextfree + n), PGSIZE);
	if (remapped_phy_mem_top >= new_nextfree) {
		result = nextfree;
...
page_init(void)
{
	...
	// the IO hole [IOPHYSMEM, EXTPHYSMEM) should never be allocated.
...
check_page_free_list(bool only_low_memory)
{
	...
	for (pp = page_free_list; pp; pp = pp->pp_link) {
		...
		// the code below checks whether you use the memory 
		// in [PGSIZE, npages_basemem * PGSIZE)
